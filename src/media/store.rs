...
#[derive(Debug)]
pub enum Error {
    Io { op: ErrorOp, source: io::Error },
    Chained { context: String, source: Box<Error> },
}
impl Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::Io { op, .. } => match op {
            ...
            Error::Chained { context, .. } => {
                f.write_fmt(format_args!("failed to process {}", context))
            }
        }
    }
}
...
impl Error {
    pub(crate) fn chained<C: Display>(context: C, source: Error) -> Error {
        Error::Chained {
    ...
    pub(crate) fn load(
        ...
        match self.load_cache.entry(hash.clone()) {
            ...
            Entry::Vacant(v) => {
                ...
                let data = Rc::new(fs::read(&p).map_err(|e| Error::Io {
        ...
        match self.store_cache.entry(hash.clone()) {
            Entry::Occupied(mut v) => {
                ...
                if m.contains(extension) {
                    ...
                    let p = hash.store_path(&self.media_root, extension);
                    if !p.exists() {
                        fs::hard_link(&pb, &p).map_err(|e| Error::Io {
                            op: ErrorOp::HardLink {
            ...
            Entry::Vacant(v) => {
                ...
                if !p.exists() {
                    if !pb.exists() {
                        fs::write(&pb, data).map_err(|e| Error::Io {
